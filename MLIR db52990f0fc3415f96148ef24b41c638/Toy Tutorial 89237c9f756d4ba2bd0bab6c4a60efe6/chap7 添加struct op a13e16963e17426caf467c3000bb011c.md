...
每一个`type`都是`TypeStorage`的实例，定义一个type，需要首先定义它的MLIRContext表示(内存表示?)，
...
定义**`StructType`   绑定**`StructTypeStorage`
...
/// (StructTypeStorage).
classStructType :public mlir::Type::TypeBase<StructType, mlir::Type,
                                               StructTypeStorage> {
public:
...
[MLIR向源语言添加struct类型](https://zhuanlan.zhihu.com/p/108575517)